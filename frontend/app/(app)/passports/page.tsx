            ...
            <div key={p.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: 18, marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 12 }}>
                ...
                <div style={{ textAlign: 'right', minWidth: 90 }}>
                  ...
                  <button onClick={() => buy(p.slug)} disabled={busy === p.slug} style={{ marginTop: 6, background: BRAND, color: '#fff', fontWeight: 800, padding: '9px 16px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, opacity: busy === p.slug ? 0.6 : 1, display: 'block', width: '100%' }}>Pay by card</button>
                </div>
              </div>
            </div>
          ))}
