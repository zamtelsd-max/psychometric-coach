      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <Link href="/advertise" className="inline-flex items-center gap-2 mb-6">
            <div className="w-10 h-10 bg-[#245599] rounded-xl flex items-center justify-center text-white font-black">P</div>
            <span className="font-bold text-[#245599] text-lg">PsychometricCoach</span>
          </Link>
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type={show ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#245599]/30 focus:border-[#245599] pr-12"
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-[#245599] text-white font-bold py-3.5 rounded-xl hover:bg-blue-900 disabled:opacity-50 transition-all">
              {loading ? 'Signing in…' : 'Sign In'}
            </button>
          </form>
          <p className="text-center text-sm text-gray-500 mt-4">
            No account? <Link href="/advertise/register" className="text-[#245599] font-semibold">Create one</Link>
          </p>
        </div>
      </div>
    </div>
